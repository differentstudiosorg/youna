...
exports.handler = function(event, context, callback) {
    ...
    if (STAGES.indexOf(stage) >= 0) {
        loadConfig(config_table, stage, context, function(env_config){
            ...
            var source = event.body.default_source;
            ...
            if (source === undefined || source === null || source === '') {
                var error = new Error("Source is either null or undefined");
                return callback(error);
            }

            async.waterfall([
                async.apply(verifyToken, token, source),
                getCustomerId,
                setDefaultSource
            ], function(err, done) {
                if (err) {
...
function verifyToken(token, source, callback) {
    ...
    verifier.verify(token, google_client_id, function (err, tokenInfo) {
      if (!err) {
        ...
        if (tokenInfo.email === undefined) {
            ...
            return callback(null, tokenInfo, source);
        }
      } else {
...
function getCustomerId(data, source, callback) {
    ...
    docClient.get(params, function(err, user_item) {
        if (err) {
            ...
            return callback(null, customer_id, source);
        }
    });

}

function setDefaultSource(customer_id, source, callback) {
    stripe.customers.update(customer_id, {
        ...
        if (err) {
            ...
            var res = { message : 'Default Source Updated' };
