...
function parseCreatorsWithPendingQuestions(creators) {
    var creators_with_pending_questions =[];
    for (var i = 0; i < creators.length; i++) {
        if (creators[i].pending_questions > 0) creators_with_pending_questions.push(creators[i]);
    }
    return creators_with_pending_questions;
}
...
function getAllCreators(callback) {
    ...
    docClient.scan(params, function(err, data) {
        if (err) {
            ...
            var creators = parseCreatorsWithPendingQuestions(data.Items);

            return callback(null, creators);
        }
    });
}

function notifyEveryone(creators, callback) {
    var num = 0;
    var num_errors = 0;
    //console.log(creators);
    async.eachSeries(creators, function(creator, callback2) {

        var params = {
            TableName: user_table,
            Key : { 'google_id' : creator.google_id}
        }
        ...
        docClient.get(params, function(err, data) {
            if (err) {
                ...
                if (user.sns_endpoint_arn != undefined) {
                    console.log(user.sns_endpoint_arn);
                    var sns = new aws.SNS();
                    var title =  "A good coach never wastes a Saturday, answer your Qs today!";
