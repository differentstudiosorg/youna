...
exports.handler = function(event, context, callback) {
    ...
    if (STAGES.indexOf(stage) >= 0) {
        loadConfig(config_table, stage, context, function(env_config){
            ...
            var question_id = event.query.question_id;
            ...
            async.waterfall([
                async.apply(findQuestion, question_id), 
                findAnswer,
                findAnswerer
            ], function(err, res) {
                if (err) {
                    ...
                    delete res.question.answerer_id;
                    delete res.question.asker_id;

                    if (res.answer != undefined) {
                        delete res.answer.asker_id;
                        delete res.answer.asnwer_id;
                        delete res.answer.answerer_id;
                    }
...
function findQuestion(question_id, callback) {
...
function findAnswerer(res, callback) {
    var params = {
        TableName: creator_table,
        Key: {
            'creator_id' : res.question.answerer_id
        }
    };
